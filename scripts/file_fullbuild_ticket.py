...
Usage:
  python3 scripts/file_fullbuild_ticket.py                 # create (or reuse FULLBUILD_ID env)
...
def main(argv: list[str]) -> int:
    L.init()
    issue_id = argv[0] if argv else os.environ.get("FULLBUILD_ID") or find_existing()
    ...
    if issue_id:
        args["id"] = issue_id
        print(f"updating existing full-build ticket {issue_id} in place")
    else:
        print("creating new full-build ticket")
    ...
    if not ident:
        ...
        return 1
    print(f"OK: full-build ticket {ident} — {url}")
